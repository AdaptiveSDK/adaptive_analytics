...
/// Tracks learning-related analytics events on the Adaptive platform.
///
...
class AdaptiveAnalytics {
  ...
  static Future<void> logStudentInactivityEvent(
          StudentInactivityEvent event) =>
      _log('logStudentInactivityEvent', event.toMap());

  /// Logs a module completion event.
  static Future<void> logModuleCompletionEvent(
          ModuleCompletionEvent event) =>
  ...
  static Future<void> logCourseEnrollmentEvent(
          CourseEnrollmentEvent event) =>
  ...
  static void _assertAndroid() {
    if (!Platform.isAndroid) {
      throw UnsupportedError(
        'adaptive_analytics_flutter only supports Android at this time.',
      );
    }
  }
}
